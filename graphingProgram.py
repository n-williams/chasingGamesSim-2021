...
for x in range(1, round(n_cols)):
    plt.plot(dataSingleC[:,0], dataSingleC[:,x], color='r', linewidth=1, alpha=0.2)
...
for x in range(1, round(n_cols)):
    plt.plot(dataSingleR[:,0], dataSingleR[:,x], color='b', linewidth=1, alpha=0.2)
...
plt.plot(dataNu[:,0], dataNu[:,3], linestyle = 'solid', color='r', label="chaser calculated $\\nu$")
plt.plot(dataNu[:,0], dataNu[:,4], linestyle = 'solid', color='b', label="runner calculated $\\nu$")
